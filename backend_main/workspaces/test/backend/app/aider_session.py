...
class AiderSession:
    ...
    def _on_readable(self):
        ...
        self._handle_raw_data(chunk)
        self._update_file_attachments(chunk)
        self._parse_and_broadcast_tokens(chunk)
        self._detect_and_broadcast_phase(chunk)
        self._track_file_progress(chunk)
        self._generate_and_broadcast_transcript(chunk)
        self._schedule_idle_check()

    def _handle_raw_data(self, chunk):
        self._push_raw(chunk)
        self._schedule(self.broadcast({"type": "raw", "data": base64.b64encode(chunk).decode("ascii")}))

    def _update_file_attachments(self, chunk):
    ...
    def _parse_and_broadcast_tokens(self, chunk):
        clean = _strip_ansi(chunk.decode(errors="replace"))
    ...
    def _detect_and_broadcast_phase(self, chunk):
        clean = _strip_ansi(chunk.decode(errors="replace"))
    ...
    def _track_file_progress(self, chunk):
        clean = _strip_ansi(chunk.decode(errors="replace"))
        if self._task_files:
    ...
    def _generate_and_broadcast_transcript(self, chunk):
        clean = _strip_ansi(chunk.decode(errors="replace"))
        prose_lines = [
