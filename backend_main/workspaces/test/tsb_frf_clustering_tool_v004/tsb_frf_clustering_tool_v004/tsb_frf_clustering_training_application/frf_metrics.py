...
class BaselineMetricsCalculator(object):
    ...
    def _extract_frequency_data(frequencywise_data, file_indices, segment_file_dict, freq_idx):
        ...
        Real and imaginary are always pulled from segment_file_dict (independent of selected features).
        """
        freq_real_data = []
        freq_imag_data = []
        freq_gain_data = []
    ...
    def _is_convex_hull_valid(self, real_array, imag_array):
        """Check if convex hull can be created for the given data"""
        hull_x, hull_y, hull_area, hull_stats = self.hull_calculator.calculate_convex_hull(
...
class ACIMetricsCalculator(object):
    ...
    def calculate_evaluation_metric_aci1(self, real_array, imag_array, gain_array,
        ...
        try:
            ...
            hull_x, hull_y, convex_hull_area, _ = self.hull_calculator.calculate_convex_hull(real_array, imag_array)
...
class FRFClusterOptimizer(object):
    """Handles optimal k selection and baseline comparison"""

    def __init__(self):
        self.logger = logging.getLogger(__name__)

    def select_optimal_cluster(self, segment_aci_scores, k1_baseline,
                            segment_name, dataset_type,
                            metrics_to_use=['ACI1', 'ACI2', 'ACI3', 'ACI4']):
        """Select optimal cluster count separately for each metric"""

        if not k1_baseline or not segment_aci_scores:
            aci_metrics = {'ACI1', 'ACI2', 'ACI3', 'ACI4'}
            if any(m in aci_metrics for m in metrics_to_use):
                self.logger.warning(
                    f"No baseline or ACI scores available for {segment_name} ({dataset_type})")
            return {'aci1': 1, 'aci2': 1, 'aci3': 1, 'aci4': 1}

        opt_dict = {}
        sorted_k = sorted([k for k in segment_aci_scores.keys() if isinstance(k, int) and k > 1])

        # Select optimal k for each metric
        metric_handlers = {
        ...
        for metric in ['ACI1', 'ACI2', 'ACI3', 'ACI4']:
            ...
            if metric in metrics_to_use:
                ...
                baseline_score = k1_baseline.get(baseline_key, 0)

                best_k, best_score = self._find_best_k_for_metric(
                    sorted_k, segment_aci_scores, avg_key, baseline_score
                )

                opt_dict[metric_lower] = best_k
                self._log_optimal_k(metric, best_k, best_score, baseline_score)
            else:
    ...
    @staticmethod
    def _find_best_k_for_metric(sorted_k, segment_aci_scores, avg_key, baseline_score):
        """Find the best k value for a specific metric"""
        best_k = 1
        best_score = baseline_score

        for k in sorted_k:
            scores = segment_aci_scores.get(k, {})
            if avg_key in scores:
                current_score = scores[avg_key]
                if current_score > baseline_score and current_score > best_score:
                    best_k = k
    ...
    def _log_optimal_k(self, metric, best_k, best_score, baseline_score):
        """Log the optimal k selection for a metric"""
        self.logger.info(f"  Optimal k for {metric} = {best_k}")
        self.logger.info(f"  Score={best_score:.4f}")
        self.logger.info(f"  Baseline={baseline_score:.4f}")
